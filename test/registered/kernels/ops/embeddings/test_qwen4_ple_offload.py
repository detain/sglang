...
def test_qwen4_ple_prefetch_buffer_is_per_size_off_sm120(monkeypatch):
    ...
    monkeypatch.setattr(qwen4_exp_module, "get_is_capture_mode", lambda: True)
    graph_three = layer._get_prefetch_buffer(3, lookup_ids.device)
    graph_five = layer._get_prefetch_buffer(5, lookup_ids.device)
    assert graph_five.data_ptr() != graph_three.data_ptr()
    assert (
        layer._get_prefetch_buffer(3, lookup_ids.device).data_ptr()
        == graph_three.data_ptr()
    )
