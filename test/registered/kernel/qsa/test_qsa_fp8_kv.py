"""FP8 KV-cache coverage for the QSA sparse attention paths.

Checks that per-tensor k/v descales are applied through the Triton prefill
(normal and chunked) entry points, the flash-attention compact gather, the
pool write path (``_store_kv`` preserving the live K/V), and the trtllm-gen
decode BMM scales.
"""
