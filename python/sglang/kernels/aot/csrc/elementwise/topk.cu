...
#pragma unroll 8
  ...
  // the threshold bin (sgl-project/sglang#36807, fixed for the JIT sibling in
  // sgl-project/sglang#38144). Overflowing candidates were dropped above
  // *together with* their fused histogram contributions, so the staged subset
  // no longer represents the threshold-bin population. Detect the overflow
  // and refine from a full-row rescan instead; the fast path below is
  // unchanged.
