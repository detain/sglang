...
# Test seam: when set, `_deterministic_inference` returns this instead of
# consulting the runtime context.  ``None`` (production) keeps the live read.
_deterministic_inference_cached = None


def _deterministic_inference() -> bool:
    if _deterministic_inference_cached is not None:
